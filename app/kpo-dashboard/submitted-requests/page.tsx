...
export default function SubmittedSupportRequests() {
  ...
  useEffect(() => {
    if (profile?.role === "admin") setSelectedKpo("overall");
    if (profile?.kpo_name)
      setSelectedKpo(currentUserRole === "admin" ? "overall" : initialKpo);
  }, [authLoading, profile]);
  ...
  const fetchSupportRequests = async () => {
    ...
    if (profile.role !== "admin" && profile.kpo_name) {
      const userKpo = profile.kpo_name.toLowerCase().replace(/\s+/g, "_");
      query = query.eq("kpo_name", userKpo);
    }
    ...
    const formattedRequests: SupportRequest[] = data.map((r: any) => {
      const kpoKey = String(r.kpo_name);
      const bf: any = beneficiaryForm as any;
      const kpoObj = bf[kpoKey];
      const kpoQuestions = Array.isArray(kpoObj?.sections)
        ? Object.values(kpoObj.sections).flatMap((s: any) => s.questions || [])
        : [];
        ...
        {currentUserRole === "admin" ? (
          ...
          <div>
            <p className={clsx(isDarkMode ? "text-gray-300" : "text-gray-700")}>
              Showing submissions for: <strong>{profile?.kpo_name}</strong>
            </p>
          </div>
        )}
      </div>
